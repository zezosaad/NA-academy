...
export class AnalyticsService {
  ...
  async trackWatchTime(studentId: string, dto: TrackWatchTimeDto): Promise<void> {
    ...
    const hasAccess = await this.accessCheckHelper.hasSubjectAccess(studentId, asset.subjectId.toString());
  ...
  async getStudentAnalytics(studentId: string) {
    ...
    const watchTimeStats = await this.watchTimeModel.aggregate([
      { $match: { studentId: sId } },
  ...
  async getPlatformAnalytics() {
    ...
    const totalWatchTime = await this.watchTimeModel.aggregate([
