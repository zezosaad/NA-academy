...
import {
  ...
  ExamSessionDocument,
  SessionStatus,
} from '../exams/schemas/exam-session.schema.js';
import {
  SubjectCode,
  SubjectCodeDocument,
...
import {
  SecurityFlag,
  SecurityFlagDocument,
  ActionTaken,
} from '../security/schemas/security-flag.schema.js';
...
export class AdminService {
  ...
  async getDashboard() {
    ...
    const [activeStudentsNow, ongoingExams, recentActivations, securityFlags] = await Promise.all([
      // 1. Active sessions
      this.sessionModel
        .countDocuments({
          isActive: true,
